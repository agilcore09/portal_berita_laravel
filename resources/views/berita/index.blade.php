                                    ...
                                    <div class="col-md-4 mt-3">
                                        <div class="card" style="width: 18rem; height: 28rem; ">
                                            <div class="img-container">
                                                <img src="{{ asset('data_blog/' . $data->gambar) }}" class="card-img-custom"
                                                    alt="{{ $data->gambar }}">
                                            </div>
                                            <div class="card-body">
                                                <h5 class="card-title cut-head">{{ $data->judul_berita }}</h5>
                                                <p class="card-text cut-off">{{ $data->body_berita }}</p>
                                                <small class="date-parse"> {{ $data->created_at }}</small>
                                            </div>
                                            <div class="pb-2">
                                                <ul class="pl-4">
                                                    <a href="{{ url('/dashboard') }}/{{ $data->slug }}"><i
                                                            class="fa-solid fa-eye text-success"></i></a>
                                                    <a href="{{ url('/delete') }}/{{ $data->slug }}"><i
                                                            class="fa-solid fa-trash text-danger"></i></a>
                                                    <a href="{{ url('/') }}/update-berita/{{ $data->slug }}"><i
                                ...
                                @if (request()->cari_judul == null)
                                    @foreach ($data as $item)
                                        <div class="col-md-4 mt-3">
                                            <div class="card" style="width: 18rem; height: 28rem; ">
                                                <div class="img-container">
                                                    <img src="{{ asset('data_blog/' . $item->gambar) }}"
                                                        class="card-img-custom" alt="{{ $item->gambar }}">
                                                </div>
                                                <div class="card-body">
